...
int aes_ecb_test()
{
	...
	BYTE ciphertext[2][16] = {
		...
		{0x59, 0x1c, 0xcb, 0x10, 0xd4, 0x10, 0xed, 0x26, 0xdc, 0x5b,
		 0xa7, 0x4a, 0x31, 0x36, 0x28, 0x70}
	};
	BYTE key[1][32] = {
...
int aes_ccm_test()
{
	...
	BYTE ciphertext[3][32 + 16] = {
		...
		{0xd2, 0xa1, 0xf0, 0xe0, 0x51, 0xea, 0x5f, 0x62, 0x08, 0x1a,
		 0x77, 0x92, 0x07, 0x3d, 0x59, 0x3d, 0x1f, 0xc6, 0x4f, 0xbf,
		 0xac, 0xcd},
		{0xe3, 0xb2, 0x01, 0xa9, 0xf5, 0xb7, 0x1a, 0x7a, 0x9b, 0x1c,
		 0xea, 0xec, 0xcd, 0x97, 0xe7, 0x0b, 0x61, 0x76, 0xaa, 0xd9,
		 0xa4, 0x42, 0x8a, 0xa5, 0x48, 0x43, 0x92, 0xfb, 0xc1, 0xb0,
		 0x99, 0x51}
	};
	BYTE iv[3][16] = {
	...
	aes_encrypt_ccm(plaintext[0], 4, assoc[0], 8, iv[0], 7, enc_buf,
			&enc_buf_len, 4, key[0], 128);
	printf("\nNONCE        : ");
	print_hex(iv[0], 7);
	printf("\nAssoc. Data  : ");
	print_hex(assoc[0], 8);
	...
	aes_decrypt_ccm(ciphertext[0], 8, assoc[0], 8, iv[0], 7, enc_buf,
			&enc_buf_len, 4, &mac_auth, key[0], 128);
	printf("\n-Ciphertext  : ");
	print_hex(ciphertext[0], 8);
	printf("\n-decrypted to: ");
	print_hex(enc_buf, enc_buf_len);
	...
	aes_encrypt_ccm(plaintext[1], 16, assoc[1], 16, iv[1], 8, enc_buf,
			&enc_buf_len, 6, key[0], 128);
	printf("\n\nNONCE        : ");
	print_hex(iv[1], 8);
	printf("\nAssoc. Data  : ");
	...
	aes_decrypt_ccm(ciphertext[1], 22, assoc[1], 16, iv[1], 8, enc_buf,
			&enc_buf_len, 6, &mac_auth, key[0], 128);
	printf("\n-Ciphertext  : ");
	...
	aes_encrypt_ccm(plaintext[2], 24, assoc[2], 20, iv[2], 12, enc_buf,
			&enc_buf_len, 8, key[0], 128);
	printf("\n\nNONCE        : ");
	print_hex(iv[2], 12);
	printf("\nAssoc. Data  : ");
	...
	aes_decrypt_ccm(ciphertext[2], 32, assoc[2], 20, iv[2], 12, enc_buf,
			&enc_buf_len, 8, &mac_auth, key[0], 128);
	printf("\n-Ciphertext  : ");
